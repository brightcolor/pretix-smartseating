...
### Fixed
- **„Beste Plätze" bei Veranstaltungsreihen (Zeitfenster):** Die Seatmap-URL
  trägt bei Subevents bereits `?subevent=<id>`; der Vorschlags-Request hängte
  ein zweites `?quantity=…` an, wodurch der Server „Invalid subevent" lieferte.
  Query-Parameter werden jetzt korrekt mit `&` angefügt.

## [0.14.1] - 2026-06-11

### Security
- **Stored-XSS-Härtung:** Reihen-/Sitzlabels im Shop-Tooltip sowie Kategorie-
  und Hintergrund-Layer-Namen im Editor werden nicht mehr in `innerHTML`
  interpoliert, sondern ausschließlich über DOM-Properties/`textContent`
  gesetzt. Farb- und Zahlenwerte aus importierten Plan-JSONs werden vor dem
  Rendern validiert bzw. numerisch erzwungen.

## [0.14.0] - 2026-06-11

### Changed
- **Vollständige deutsche Übersetzung:** Editor, Shop und Import-Dialog komplett auf Deutsch
  umgestellt — alle Toolbar-Beschriftungen, Sidebar-Labels, Kontextmenü-Einträge,
  Validierungsmeldungen, Fehlertexte und Shop-Statuszeilen.
- Toolbar-Buttons kompakt umbenannt (z.B. „Select" → „Wählen", „Booth" → „Box",
  „Focus" → „Fokus") damit die Leiste nicht überläuft.
...
### Changed
- **Editor seat numbers are drawn on the seats** (centred in the circle, white
  with a dark outline for contrast) instead of beside them — matching the shop.
...
### Changed
- Tables and blocks are now **grouped automatically when placed** — a placed
  table becomes a "Table T1" group, a block a "Block A" group, so the whole
  unit can be selected / moved / renamed at once right away.
